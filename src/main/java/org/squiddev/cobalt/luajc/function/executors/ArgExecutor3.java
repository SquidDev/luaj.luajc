...
public abstract class ArgExecutor3 extends FunctionExecutor {
	@Override
	public Varargs execute(LuaState state, FunctionWrapper function, Varargs varargs) {
		return execute(state, function, varargs.first(), varargs.arg(2), varargs.arg(3));
	}

	@Override
	public final LuaValue execute(LuaState state, FunctionWrapper function) {
		return execute(state, function, Constants.NIL, Constants.NIL, Constants.NIL);
	}

	@Override
	public final LuaValue execute(LuaState state, FunctionWrapper function, LuaValue arg1) {
		return execute(state, function, arg1, Constants.NIL, Constants.NIL);
	}

	@Override
	public final LuaValue execute(LuaState state, FunctionWrapper function, LuaValue arg1, LuaValue arg2) {
		return execute(state, function, arg1, arg2, Constants.NIL);
	}
}
