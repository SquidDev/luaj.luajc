...
import org.squiddev.cobalt.luajc.utils.DumpInstructions;
import org.squiddev.cobalt.luajc.utils.IntArray;

import java.util.*;

import static org.squiddev.cobalt.luajc.lasm.Lexer.Token;
import static org.squiddev.cobalt.luajc.lasm.Lexer.TokenType;
import static org.squiddev.cobalt.luajc.utils.DumpInstructions.ArgType;
...
public final class LasmParser {
	...
	public LasmParser(TokenStream stream) {
		this.stream = stream;
		state = new State();
		state.proto.source = ValueFactory.valueOf("?");
	}

	public LasmParser(String name, String contents) {
		this(new TokenStream(name, contents));
		state.proto.source = ValueFactory.valueOf(name);
	}
	...
	private class State {
		...
		private void finish() {
			...
			proto.locvars = new LocalVariable[0];
	...
	private int addConstant() {
		...
		switch (token.token) {
			case STRING:
				stream.get();
				return state.getConstant(ValueFactory.valueOf(token.contents));
			case NUMBER:
				stream.get();
				return state.getConstant(ValueFactory.valueOf(Double.parseDouble(token.contents)));
			case KEYWORD:
				if (token.contents.equals("nil")) {
					stream.get();
					return state.getConstant(Constants.NIL);
				} else if (token.contents.equals("true")) {
					stream.get();
					return state.getConstant(Constants.TRUE);
				} else if (token.contents.equals("false")) {
					stream.get();
					return state.getConstant(Constants.FALSE);
				}
				break;
		}
	...
	private void doControl() {
		if (stream.consumeKeyword(".const")) {
			...
			if (stream.is(TokenType.STRING) || stream.is(TokenType.IDENTIFIER)) {
				state.proto.source = ValueFactory.valueOf(stream.get().contents);
			} else {
			...
			if (stream.is(TokenType.STRING) || stream.is(TokenType.IDENTIFIER)) {
				state.upvalues.add(ValueFactory.valueOf(stream.get().contents));
			} else {
				throw parseError("Expected string or identifier");
			}
	...
	private int parseNumber(boolean isRK) {
		...
		if (stream.consumeIdent("k") || stream.consumeIdent("const")) {
			...
			return offset + state.getConstant(Constants.TRUE);
		...
		switch (type) {
			...
			case Closure:
				if (stream.is(TokenType.NUMBER)) {
					...
					LuaValue value = ValueFactory.valueOf(stream.peek().contents);
					int i = 0;
